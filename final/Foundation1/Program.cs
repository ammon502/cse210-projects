...
class Program
{
    ...
    {   
        ...
        {
            System.Console.WriteLine($"Video {_videoCount} created by {v.getAuthor()} is {v.getLength} minutes long:"); //display non comment properties of the video
            v.setComment(); // filling in the comments

            System.Console.Write($"Comment {commentCount}: ");
            v.getComment(); // displaying the comments
            commentCount++;
            
            System.Console.Write($"Video {_videoCount} has {v.numComments()} comments");//
            _videoCount++;
        }
    }
}